...
function Tqareer18() {
    ...
    const handleSearch = async () => {
        try {
            const response = await axios.get("http://30.30.30.53:3000/reports/officers_cases/", {
                params: {
                    date_from: dateFrom,
                    date_to: dateTo,
                    class_in: classIn,
    ...
    const generatePDF = async () => {
        ...
        for (let start = 0; start < officers.length; start += rowsPerPage) {
            ...
            <table style="border-collapse: collapse; width: 100%;">
                <thead>
                    <tr>
                        <th style="border: 1px solid #000; padding: 8px;">المسلسل</th>
                        <th style="border: 1px solid #000; padding: 8px;">الرقم </th>
                        <th style="border: 1px solid #000; padding: 8px;">الرقم </th>
                        <th style="border: 1px solid #000; padding: 8px;">الرتبة</th>
                        <th style="border: 1px solid #000; padding: 8px;">الإسم</th>
                ...
                <tbody>
                    ${chunk.map((officer, index) => `
                        <tr>
                            ...
                            <td style="border: 1px solid #000; padding: 8px;">${officer.MIL_NO}</td>
                            <td style="border: 1px solid #000; padding: 8px;">${officer.SENIORITY_NAME}</td>
                            <td style="border: 1px solid #000; padding: 8px;">${officer.RNK}</td>
                            <td style="border: 1px solid #000; padding: 8px;">${officer.OFF_NAME}</td>
                            <td style="border: 1px solid #000; padding: 8px;">${officer.CRP}</td>
                            <td style="border: 1px solid #000; padding: 8px;">${(new Date(officer.CASE_DATE)).toLocaleDateString('en-CA')}</td>
                            <td style="border: 1px solid #000; padding: 8px;">${officer.CASE_DETAIL}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
        ...
        link.href = url;
        link.download = `طباعة_المحالين_لقضايا_${new Date().toLocaleDateString()}.pdf`;
        link.click();
                    ...
                    <table className="w-full border border-collapse border-black">
                        <thead>
                            <tr>
                                ...
                                <th className="px-4 py-2 border border-black">الرقم الاقدمية</th>
                                <th className="px-4 py-2 border border-black">الرتبة</th>
                                <th className="px-4 py-2 border border-black">الإسم</th>
                                <th className="px-4 py-2 border border-black">السلاح</th>
                                <th className="px-4 py-2 border border-black">التاريخ</th>
                                <th className="px-4 py-2 border border-black">البيان</th>
                            </tr>
                        </thead>
                        <tbody>
                            {officers.map((officer, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-4 py-2 border border-black">{officer.CRP}</td>
