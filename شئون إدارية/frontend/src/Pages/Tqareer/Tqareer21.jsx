...
function Tqareer21() {
    ...
    const [officersTi, setOfficersTi] = useState([]);
    ...
    useEffect(() => {
        const fetchOfficers = async () => {
            try {
                const response = await axios.get("http://30.30.30.53:3000/reports/estekmalpersentage/?unit_no=6663");
                setOfficers(response.data.units);
                setOfficersTi(response.data.title);
            } catch (error) {
    ...
    const generateAllPagesPDF = async () => {
        ...
        const fileName = `اقارب_الظباط_${new Date().toLocaleDateString()}.pdf`;
        const pdfBlob = pdf.output('blob');
        const url = URL.createObjectURL(pdfBlob);
        const newTab = window.open(url, '_blank');
        const link = document.createElement('a');
                    ...
                    <tbody>
                        {paginatedOfficers.map((officer, index) => (
                            <tr key={index}>
                                ...
                                {/* <td className="px-4 py-2 border border-black">{officer.title}</td>  */}
                                <td className="px-4 py-2 border border-black">{officer.UNITNO}</td>
                                <td className="px-4 py-2 border border-black">{officer.LEVEL}</td>
                                <td className="px-4 py-2 border border-black">{officer.UNITNAME}</td>
                                <td className="px-4 py-2 border border-black">{officer.moratap3amel}</td>
                                <td className="px-4 py-2 border border-black">{officer.exists3amel}</td>
                                <td className="px-4 py-2 border border-black">{officer.ehtiatExists3amel}</td>
                                <td className="px-4 py-2 border border-black">{officer.stikmalPersentage3amel}</td>
                                <td className="px-4 py-2 border border-black">{officer.moratapFany}</td>
                                <td className="px-4 py-2 border border-black">{officer.existsFany}</td>
                                <td className="px-4 py-2 border border-black">{officer.stikmalPersentageFany}</td>
                                <td className="px-4 py-2 border border-black">{officer.moratapSharaf}</td>
                                <td className="px-4 py-2 border border-black">{officer.existsSharaf}</td>
                                <td className="px-4 py-2 border border-black">{officer.stikmalPersentageSharaf}</td>
                                <td className="px-4 py-2 border border-black">{officer.unitSumMoratap}</td>
                                <td className="px-4 py-2 border border-black">{officer.unitSumExists}</td>
                                <td className="px-4 py-2 border border-black">{officer.unitSumStikmalPersentage}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
