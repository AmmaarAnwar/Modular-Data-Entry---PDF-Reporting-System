...
const Tqareer9 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://30.30.30.53:3000/reports/co_efficiency_report/");
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://30.30.30.53:3000/reports/all_efficiency_report/?${queryParams}`, {
        method: "GET",
        headers: {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-600 bg-opacity-50">
          <div className="relative w-full p-6 bg-white rounded-lg shadow-lg max-w-7xl">
            ...
            <img
              src="/.png"
              alt="Watermark"
              className="absolute inset-0 z-10 object-cover pr-96 opacity-15"
              ...
              <table className="z-20 w-full bg-white border border-gray-300">
                <thead>
                  <tr className="w-full bg-gray-100 border-b">
                    <th className="p-2 px-4 py-2 text-center border"> ش </th>
                    <th className="p-2 px-4 py-2 text-center border"> ش </th>
