...
function Edkhal28() {
  ...
  let postObject = { "JUDGMENT_DATE": null, "NO_OF_IMAGES": null, "MIL_NO": null, "CASE_YEAR": null, "CASE_NO": null };
  ...
  return (
    <>
      ...
      <div className="pb-3 text-4xl font-black text-center text-gray-900 dark:text-white">
        <p>تعديل عدد الصفحات --حكم محكمة--</p>
      </div>
        <h6>المحاكمات</h6>
            ...
            <button onClick={
              async () => {
                ...
                if (!(document.getElementById("MIL_NO").value == "")) {
                  const response = await axios.get(`http://30.30.30.53:3000/edkhal/edkhal28?mil_no=${document.getElementById("MIL_NO").value}`);
        ...
        <div className="flex">
          ...
          <input
            name="JUDGMENT_DATE"
            value={values.JUDGMENT_DATE && new Date(values.JUDGMENT_DATE).toISOString().split('T')[0]}
            onChange={(ele) => {
              handleChange(ele)
            }}
