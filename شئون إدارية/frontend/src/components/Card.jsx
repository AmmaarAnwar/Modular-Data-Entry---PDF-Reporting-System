...
function Card() {
    ...
    if (!hasFetched) {
      const fetchData = async () => {
        try {
          ...
          const { TOTAL_OFFICERS, KHEDMA, MA3ASH } = data.data;
          setChartData({ total: TOTAL_OFFICERS, series: [KHEDMA, MA3ASH] });
          setHasFetched(true);
        } catch (error) {
  ...
  useEffect(() => {
    ...
    if (chartData.series.length && hasFetched) {
      const chart = new ApexCharts(document.getElementById("donut-chart"), getChartOptions(chartData));
      chart.render();
